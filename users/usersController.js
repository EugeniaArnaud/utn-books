...
const register = async(req, res, next) => {
    ...
    const dbResponse = await addNewUser({ name, email, password }) //sacar image
    if (dbResponse instanceof Error) return next(dbResponse);
...
const editOne = async(req, res, next) => {
    if (notNumber(req.params.id, next)) return
        // const image = `${public_url}/${req.file.filename}` image ??
    const dbResponse = await editUserById(+req.params.id, {...req.body }) //sacar solamente image?
    if (dbResponse instanceof Error) return next(dbResponse)
